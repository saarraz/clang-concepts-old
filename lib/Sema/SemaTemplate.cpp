...
static ExprResult formImmediatelyDeclaredConstraint(
  ...
  ConstraintArgs.addArgument(
    TemplateArgumentLoc(TemplateArgument(ConstrainedType),
                        S.Context.getTrivialTypeSourceInfo(ConstrainedType,
                                                           ParamNameLoc)));
  ...
  CXXScopeSpec SS;
  SS.Adopt(NS);
  ExprResult ImmediatelyDeclaredConstraint = S.CheckConceptTemplateId(SS,
...
NamedDecl *Sema::ActOnNonTypeTemplateParameter(Scope *S, Declarator &D,
  ...
  if (AutoTypeLoc TL = TInfo->getTypeLoc().findAutoTypeLoc())
    if (TL.isConstrained())
...
bool Sema::CheckTemplateArgument(TemplateTemplateParmDecl *Param,
  ...
  if (getLangOpts().RelaxedTemplateTemplateArgs || getLangOpts().ConceptsTS) {
    ...
    if (isTemplateTemplateParameterAtLeastAsSpecializedAs(Params, Template,
      ...
      if (DeclContext *TemplatedEntityParent =
              Param->getDeclContext()->getParent())
        ParamMLTAL =
            getTemplateInstantiationArgs(cast<Decl>(TemplatedEntityParent),
                                         /*Innermost=*/&ParamsSoFar);
      else
        ParamMLTAL.addOuterTemplateArguments(&ParamsSoFar);

...
bool r24 = requires {
    ...
    typename identity<T>::template temp; // expected-error{{expected identifier or template-id in type requirement}}
};
...
bool r30 = requires { { 0 } noexcept -> const volatile C1 auto; };

bool r31 = requires { { 0 } noexcept -> const C1 auto volatile; };

bool r32 = requires { { 0 } noexcept -> C1 auto const volatile; };

bool r33 = requires { { 0 } noexcept -> const volatile C1 auto const volatile; }; // expected-warning {{duplicate 'const' declaration specifier}} expected-warning {{duplicate 'volatile' declaration specifier}}
#endif

template<typename T>
T i1 = 0;

bool r34 = requires { { 0 } noexcept -> const volatile i1; };
// expected-error@-1 {{unknown type name 'i1'}}

bool r35 = requires { { 0 } noexcept -> const int; };

bool r36 = requires { { 0 } noexcept -> const volatile decltype(i1<int>); };

template<typename T, typename U>
concept C2 = true;

#if 0 // TODO: Concepts: Re-enable once we have placeholder constraints
bool r37 = requires { { 0 } noexcept -> const volatile C2 auto; };
...
bool r38 = requires { { 0 } noexcept -> const volatile C1<> auto; };

bool r39 = requires { { 0 } noexcept -> const volatile C2<int> auto; };
#endif

bool r40 = requires { requires false, 1; };
// expected-error@-1 {{expected ';' at end of requirement}}

bool r41 = requires { 0 noexcept; };
...
bool r44 = requires (bool b) { requires sizeof(b) == 1; };

// TODO: Concepts: Enable once we have placeholder constraints.

bool r45 = requires { { 0 } -> auto&&; };
// expected-error@-1 {{'auto' not allowed in expression type requirement}}

bool r46 = requires { { 0 } -> decltype(auto); };
// expected-error@-1 {{'decltype(auto)' not allowed in expression type requirement}}

bool r47 = requires { { 0 } -> const auto *; };
// expected-error@-1 {{'auto' not allowed in expression type requirement}}

void r48(bool b) requires requires { 1 } {}
// expected-error@-1 {{expected ';' at end of requirement}}
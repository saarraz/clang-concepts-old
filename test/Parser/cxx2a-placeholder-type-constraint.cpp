...
int foo() {
  ...
  // expected-error@-1{{non-const lvalue reference to type 'int' cannot bind to a temporary of type 'int'}}
  C a15 = 1;
  // expected-error@-1{{ISO C++2a requires 'auto' after a concept name for placeholders}}
  C decltype a19 = 1;
  // expected-error@-1{{expected '('}}
  C decltype(1) a20 = 1;
  // expected-error@-1{{only 'decltype(auto)' or 'auto' may appear after a concept name for placeholders}}
}
...
false_v<requires (T t) { requires is_same_v<decltype(t), int>; }> // expected-note {{because 'false_v<requires (int t) { requires is_same_v<decltype(t), int>; }>' evaluated to false}} expected-note {{because 'false_v<requires (char t) { requires is_same_v<decltype(t), int>; }>' evaluated to false}}
struct r1 {};
...
false_v<requires (Ts... ts) {requires ((sizeof(ts) == 2) && ...);}> // expected-note{{because 'false_v<requires (short ts, unsigned short ts) { requires (sizeof (ts) == 2) && (sizeof (ts) == 2); }>'}} expected-note{{because 'false_v<requires (short ts) { requires (sizeof (ts) == 2); }>' evaluated to false}}
struct r2 {};
...
template<typename... Ts> requires
false_v<(requires (Ts ts) {requires sizeof(ts) != 0;} && ...)> // expected-note{{because 'false_v<requires (short ts) { requires sizeof (ts) != 0; } && requires (unsigned short ts) { requires sizeof (ts) != 0; }>' evaluated to false}} expected-note{{because 'false_v<requires (short ts) { requires sizeof (ts) != 0; }>' evaluated to false}}
struct r3 {};
...
namespace type_requirement {
  ...
  template<typename T> requires
  false_v<requires { typename T::template temp<T>; }> // expected-note{{because 'false_v<requires { typename contains_template<int>::temp<contains_template<int> >; }>' evaluated to false}} expected-note{{because 'false_v<requires { typename contains_template<short>::temp<contains_template<short> >; }>' evaluated to false}}
  struct r2 {};
  ...
  template<typename T>
  struct a {
      template<typename U> requires (requires { typename T::a::a; }, false) // expected-note{{because 'requires { typename <<error-type>>; } , false' evaluated to false}}
      struct r {};
  };
  ...
  template<typename... Ts> requires
  false_v<(requires { typename Ts::type; } && ...)> // expected-note{{because 'false_v<requires { typename identity<short>::type; } && requires { typename identity<int>::type; } && requires { typename <<error-type>>; }>' evaluated to false}}
  struct r5 {};
  ...
  template<typename... Ts> requires
  false_v<(requires { typename Ts::template aaa<Ts>; } && ...)> // expected-note{{because 'false_v<requires { typename <<error-type>>; } && requires { typename <<error-type>>; }>' evaluated to false}}
  struct r7 {};
...
namespace expr_requirement {
  ...
  template<typename T> requires false_v<requires { sizeof(T); { sizeof(T) }; }> // expected-note{{because 'false_v<requires { sizeof(int); { sizeof(int) }; }>' evaluated to false}} expected-note{{because 'false_v<requires { <<error-expression>>; { <<error-expression>> }; }>' evaluated to false}}
  struct r1 {};
  ...
  template<typename T> requires
  false_v<requires { { sizeof(T) } -> typename T::type; }> // expected-note{{because 'false_v<requires { { sizeof(identity<int>) } -> typename identity<int>::type; }>' evaluated to false}}
  struct r2 {};
  ...
  template<typename T>
  concept C1 = is_same_v<T, int>;

  template<typename T> requires false_v<requires(T t) { { t } -> C1<T>; }> // expected-note{{because 'false_v<requires (int t) { { t } -> C1<int>; }>' evaluated to false}} expected-note{{because 'false_v<requires (double t) { { t } -> C1<double>; }>' evaluated to false}}
  struct r3 {};
  ...
  template<typename T> requires
  false_v<requires(T t) { { foo } -> C1(int, T); }> // expected-note{{because 'false_v<requires (double t) { { foo } -> C1 (int, double); }>' evaluated to false}} expected-note{{because 'false_v<requires (int t) { { foo } -> C1 (int, int); }>' evaluated to false}}
  struct r4 {};
  ...
  // Parameter pack inside expr
  template<typename... Ts> requires
  false_v<(requires { { sizeof(Ts) } noexcept -> int; } && ...)> // expected-note{{because 'false_v<requires { { sizeof(int) } noexcept -> int; } && requires { { sizeof(short) } noexcept -> int; }>' evaluated to false}}
  struct r5 {};
  ...
  template<typename... Ts> requires
  false_v<(requires { { 0 } noexcept -> Ts; } && ...)> // expected-note{{because 'false_v<requires { { 0 } noexcept -> int; } && requires { { 0 } noexcept -> unsigned int; } && requires { { 0 } noexcept -> long; }>' evaluated to false}}
  struct r7 {};
  ...
  template<typename... Ts> requires
  false_v<(requires { { 0 } noexcept -> const C1(Ts); } && ...)> // expected-note{{because 'false_v<requires { { 0 } noexcept -> const C1 (int); } && requires { { 0 } noexcept -> const C1 (unsigned int); }>' evaluated to false}}
  struct r8 {};
  ...
  template<typename... Ts> requires
  false_v<(requires { { 0 } noexcept -> C2<Ts>[3]; } && ...)> // expected-note{{because 'false_v<requires { { 0 } noexcept -> C2<int> [3]; } && requires { { 0 } noexcept -> C2<unsigned int> [3]; }>' evaluated to false}}
  struct r9 {};

  using r9i = r9<int, unsigned int>; // expected-error{{constraints not satisfied for class template 'r9' [with Ts = <int, unsigned int>]}}

  template<typename... Ts> requires
  false_v<(requires { { sizeof(Ts) } noexcept -> C2<Ts>[sizeof(Ts)]; } && ...)> // expected-note{{because 'false_v<requires { { sizeof(int) } noexcept -> C2<int> [4]; } && requires { { sizeof(long) } noexcept -> C2<long> [8]; }>' evaluated to false}}
  struct r10 {};

  using r10i = r10<int, long>; // expected-error{{constraints not satisfied for class template 'r10' [with Ts = <int, long>]}}

  template<typename... Ts> requires
  false_v<(requires { { sizeof(Ts) } noexcept -> const volatile Ts; } && ...)> // expected-note{{because 'false_v<requires { { sizeof(int) } noexcept -> const volatile int; } && requires { { sizeof(unsigned int) } noexcept -> const volatile unsigned int; }>' evaluated to false}}
  struct r11 {};
...
namespace nested_requirement {
  ...
  // Parameter pack inside expr
  template<typename... Ts> requires
  false_v<(requires { requires sizeof(Ts) == 0; } && ...)> // expected-note{{because 'false_v<requires { requires sizeof(int) == 0; } && requires { requires sizeof(short) == 0; }>' evaluated to false}}
  struct r2 {};
...
template<typename... Ts> requires
false_v<(requires { requires sizeof(Ts) == 0; sizeof(Ts); } && ...)> // expected-note{{because 'false_v<requires { requires sizeof(int) == 0; sizeof(int); } && requires { requires sizeof(short) == 0; sizeof(short); }>' evaluated to false}}
struct r4 {};

using r4i = r4<int, short>; // expected-error{{constraints not satisfied for class template 'r4' [with Ts = <int, short>]}}

template<typename... Ts> requires
false_v<(requires(Ts t) { requires sizeof(t) == 0; t++; } && ...)> // expected-note{{because 'false_v<requires (int t) { requires sizeof (t) == 0; t++; } && requires (short t) { requires sizeof (t) == 0; t++; }>' evaluated to false}}
struct r5 {};

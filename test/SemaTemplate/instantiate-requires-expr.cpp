...
namespace expr_requirement {
  ...
  using r6i = r6<int, unsigned int, long>; // expected-error{{constraints not satisfied for class template 'r6' [with Ts = <int, unsigned int, long>]}}

  template<typename T, typename U>
  concept C2 = true;

  template<typename... Ts> requires
  false_v<(requires { { 0 } noexcept -> C2<Ts>; } && ...)>
  // expected-note@-1 {{because 'false_v<requires { { 0 } noexcept -> C2<int>; } && requires { { 0 } noexcept -> C2<unsigned int>; }>' evaluated to false}}
  struct r7 {};

  using r7i = r7<int, unsigned int>; // expected-error{{constraints not satisfied for class template 'r7' [with Ts = <int, unsigned int>]}}

...
#if 0 // TODO: Concepts re-enable when we have placeholders.

template<typename T, typename U> requires requires (T t) { { t } -> const Same<U> auto*; } // expected-note{{because expression type 'const int' does not match given pattern 'const <type> *'}}
struct r12 {};
...
template<typename T, typename U> requires requires (T t) { { t } -> const Same<U> auto volatile; }
struct r13 {};
...
template<typename T, typename U> requires requires (T t) { { t } -> Same<U> auto**; } // expected-note{{because expression type 'const short *' does not match given pattern '<type> **'}}
struct r14 {};
...
template<typename T> requires requires (T t) { { t } -> IsEven; } // expected-error{{concept named in type constraint must be a type concept}}
struct r18 {};
...
namespace std_example {
...
#if 0 // TODO: Concepts: Reenable when we have placeholder constraints
  ...
  template<typename T> concept C4 =
    requires(T x) {
      {*x} -> C3<int> auto const&; // expected-note{{because '*x' would be invalid: indirection requires pointer operand ('int' invalid)}} expected-note {{because type constraint 'C3<int, int>' was not satisfied:}} expected-note {{because type constraint 'C3<short, int>' was not satisfied:}}
    };
